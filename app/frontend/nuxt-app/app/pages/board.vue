    ...
    <div class="dashboard">
        ...
        <div class="rounded-lg ring ring-default  filters-panel">
            ...
            <div class="filter-group" v-if="shouldShowRegionInput">
                ...
                <input id="region" type="text" v-model="filters.regionName" @input="onAutocompleteInput('region')"
                    @focus="onAutocompleteFocus('region')" placeholder="Например: Москва" />
                <div v-if="showAutocomplete.region" class="autocomplete-dropdown">
                    <div v-for="result in autocompleteResults.region" :key="result" class="autocomplete-item"
                        @click="selectAutocomplete('region', result)">
                        {{ result }}
                    </div>
                </div>
            </div>
            ...
            <div class="filter-group" v-if="shouldShowSettlementInput">
                ...
                <input id="settlement" type="text" v-model="filters.settlementName"
                    @input="onAutocompleteInput('settlement', $event)" @focus="onAutocompleteFocus('settlement')"
                    placeholder="Например: Железнодорожный" />
                <div v-if="autocompleteResults.settlement.length > 0 && showAutocomplete.settlement"
                    class="autocomplete-dropdown">
                    <div v-for="result in autocompleteResults.settlement" :key="result" class="autocomplete-item"
                        @click="selectAutocomplete('settlement', result)">
                        {{ result }}
                    </div>
                </div>
            </div>
            ...
            <div class="filter-group" v-if="shouldShowDistrictInput">
                ...
                <input id="district" type="text" v-model="filters.districtName"
                    @input="onAutocompleteInput('district', $event)" @focus="onAutocompleteFocus('district')"
                    placeholder="Например: Центральный" />
                <div v-if="autocompleteResults.district.length > 0 && showAutocomplete.district"
                    class="autocomplete-dropdown">
                    <div v-for="result in autocompleteResults.district" :key="result" class="autocomplete-item"
                        @click="selectAutocomplete('district', result)">
                        {{ result }}
                    </div>
                </div>
            </div>
            ...
            <div class="filter-group" v-if="shouldShowMicrodistrictInput">
                ...
                <input id="microdistrict" type="text" v-model="filters.microdistrictName"
                    @input="onAutocompleteInput('microdistrict', $event)" @focus="onAutocompleteFocus('microdistrict')"
                    placeholder="Например: Серебрянка" />
                <div v-if="autocompleteResults.microdistrict.length > 0 && showAutocomplete.microdistrict"
                    class="autocomplete-dropdown">
                    <div v-for="result in autocompleteResults.microdistrict" :key="result" class="autocomplete-item"
                        @click="selectAutocomplete('microdistrict', result)">
                        {{ result }}
                    </div>
                </div>
            </div>
           ...
           <div class="filter-group">
            ...
            <div class="filter-group">
                <button @click="addToComparison" class="comparison-btn">
            ...
            <div class="comparison-list">
                <div v-for="(item, index) in comparisonItems" :key="item.id" class="comparison-item">
                    <div class="comparison-header">
                        <button @click="removeFromComparison(item.id)" class="remove-btn">
                            ✕
                        </button>
                    </div>
...
const settlementTypeOptions = ref<CheckboxGroupItem[]>([{ value: 'город', label: 'Город' }, { value: 'поселок', label: 'Поселок' }, { value: 'деревня', label: 'Деревня' }])
