<template>
    ...
    <div class="filters-content">
        <div class="filters-header">
            <p class="font-semibold text-lg">Фильтры
                <UBadge v-if="activeFiltersCount > 0" color="info" class="ml-2">
                    {{ activeFiltersCount }}
                </UBadge>
            </p>
        ...
        <div v-else class="overflow-auto">
            <div class="filter-section p-5 border-b border-b-neutral-200">
                <h4>Тип недвижимости</h4>
                <div class="button-group">
                    <UButton v-for="type in propertyTypeOptionsDynamic" :key="type.value"
                        :variant="filters.property_type === type.value ? 'solid' : 'outline'"
                        :color="filters.property_type === type.value ? 'neutral' : 'neutral'"
                        @click="setPropertyType(type.value)" size="sm">
                        {{ type.label }}
                    </UButton>
                </div>
            </div>
            <div class="filter-sections">
                ...
                <div v-if="!isHouse" class="filter-section">
                    <h4>Тип объекта</h4>
                    <div class="button-group">
                        <UButton v-for="option in flatTypeOptions" :key="option.value"
                            :variant="filters.is_new_house === option.value ? 'solid' : 'outline'"
                            :color="filters.is_new_house === option.value ? 'primary' : 'neutral'"
                            @click="setPropertyTypeFilter(option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                    <h4 v-if="filters.is_new_house">Дополнительно </h4>
                    <UCheckbox v-if="filters.is_new_house" :model-value="filters.is_build_complete"
                        @update:modelValue="v => setBooleanFilter('is_build_complete', v)" label="Дом сдан" />
                </div>
                ...
                <div class="filter-section">
                    ...
                    <div class="button-group flex-col">
                        <UInput v-model.number="filters.min_price_per_square_meter" type="number"
                            placeholder="Мин. цена за м²" />
                        <UInput v-model.number="filters.max_price_per_square_meter" type="number"
                ...
                <div v-if="!isFlat" class="filter-section">
                    <h4>Спальни</h4>
                    <div class="button-group">
                        <UButton v-for="bedroom in [1, 2, 3, 4, 5]" :key="bedroom"
                            ...
                            @click="toggleArrayFilter('bedrooms_count', bedroom)" size="sm">
                            {{ bedroom }}
                        </UButton>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    ...
                    <div class="button-group flex-col">
                        <UInput v-model.number="filters.min_house_floors_count" type="number"
                            placeholder="Мин. кол-во этажей" />
                        <UInput v-model.number="filters.max_house_floors_count" type="number"
                ...
                <div v-if="!isHouse" class="filter-section">
                    ...
                    <div class="button-group">
                        <UButton v-for="option in filteredRenovationOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('renovation_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    <h4>Тип санузла</h4>
                    <div class="button-group">
                        <UButton v-for="option in filteredBathroomOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('bathroom_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div v-if="!isFlat" class="filter-section">
                    <h4>Коммуникации</h4>
                    <div class="button-group flex-col">
                        <UCheckbox :model-value="filters.has_water_supply"
                            @update:modelValue="v => setBooleanFilter('has_water_supply', v)" label="Вода" />
                        <UCheckbox :model-value="filters.has_electricity"
                            @update:modelValue="v => setBooleanFilter('has_electricity', v)" label="Электричество" />
                        <UCheckbox :model-value="filters.has_gas"
                            @update:modelValue="v => setBooleanFilter('has_gas', v)" label="Газ" />
                        <UCheckbox :model-value="filters.has_sewerage"
                            @update:modelValue="v => setBooleanFilter('has_sewerage', v)" label="Канализация" />
                        <UCheckbox :model-value="filters.has_heating"
                            @update:modelValue="v => setBooleanFilter('has_heating', v)" label="Отопление" />
                    </div>
                </div>
                ...
                <div class="filter-section">
                    ...
                    <div class="button-group flex-col">
                        <template v-if="!isHouse">
                            <UCheckbox :model-value="filters.has_furniture"
                                @update:modelValue="v => setBooleanFilter('has_furniture', v)" label="Мебель" />
                            <UCheckbox :model-value="filters.has_elevator"
                                @update:modelValue="v => setBooleanFilter('has_elevator', v)" label="Лифт" />
                            <UCheckbox :model-value="filters.has_balcony"
                                @update:modelValue="v => setBooleanFilter('has_balcony', v)" label="Балкон" />
                            <UCheckbox :model-value="filters.has_garbage_chute"
                                @update:modelValue="v => setBooleanFilter('has_garbage_chute', v)"
                                label="Мусоропровод" />
                        </template>
                        <template v-else-if="!isFlat">
                            <UCheckbox :model-value="filters.has_guard"
                                @update:modelValue="v => setBooleanFilter('has_guard', v)" label="Охрана" />
                            <UCheckbox :model-value="filters.has_garage"
                                @update:modelValue="v => setBooleanFilter('has_garage', v)" label="Гараж" />
                            <UCheckbox :model-value="filters.has_bathhouse"
                                @update:modelValue="v => setBooleanFilter('has_bathhouse', v)" label="Баня" />
                            <UCheckbox :model-value="filters.has_pool"
                                @update:modelValue="v => setBooleanFilter('has_pool', v)" label="Бассейн" />
                            <UCheckbox :model-value="filters.has_terrace"
                                @update:modelValue="v => setBooleanFilter('has_terrace', v)" label="Терраса" />
                        </template>
                    </div>
                </div>
                ...
                <div v-if="!isHouse" class="filter-section">
                    <h4>Вид из окон</h4>
                    <div class="button-group">
                        <UButton v-for="option in windowViewTypeOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('window_view_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div v-if="!isHouse" class="filter-section">
                    <h4>Тип парковки</h4>
                    <div class="button-group">
                        <UButton v-for="option in parkingTypeOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('parking_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    <h4>Дополнительно</h4>
                    <div class="button-group flex-col">
                        <UInput v-model.number="filters.min_house_built_year" type="number"
                            placeholder="Год постройки от" />
                        <UInput v-model.number="filters.max_house_built_year" type="number"
                            placeholder="Год постройки до" />
                    </div>
                    <div class="button-group flex-col">
                        <UInput v-model.number="filters.min_ceiling_height" type="number"
                            placeholder="Высота потолков от" step="0.1" />
                        <UInput v-model.number="filters.max_ceiling_height" type="number"
                            placeholder="Высота потолков до" step="0.1" />
                    </div>
                </div>
                ...
                <div class="filter-section">
                    ...
                    <div class="button-group">
                        <UButton v-for="option in filteredHouseMaterialOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('house_material_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div v-if="!isFlat" class="filter-section">
                    <h4>Тип участка</h4>
                    <div class="button-group">
                        <UButton v-for="option in landTypeOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('land_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    <h4>Тип отопления</h4>
                    <div class="button-group">
                        <UButton v-for="option in filteredHeatingOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('heating_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div v-if="isHouse" class="filter-section">
                    ...
                    <div class="button-group">
                        <UButton v-for="option in sewerageTypeOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('sewerage_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div v-if="isHouse" class="filter-section">
                    ...
                    <div class="button-group">
                        <UButton v-for="option in waterSupplyTypeOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('water_supply_type', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    ...
                    <div class="button-group">
                        <UButton v-for="option in priceCategoryOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('price_category', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    ...
                    <div class="button-group">
                        <UButton v-for="option in elderlyCategoryOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('elderly_category', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    ...
                    <div class="button-group">
                        <UButton v-for="option in familyCategoryOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('family_category', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    ...
                    <div class="button-group">
                        <UButton v-for="option in transportAccessCategoryOptions" :key="option.value"
                            ...
                            @click="toggleArrayFilter('transport_access_category', option.value)" size="sm">
                            {{ option.label }}
                        </UButton>
                    </div>
                </div>
        ...
        <!-- Кнопка применения -->
        <div class="filters-header border-t border-t-neutral-200">
            <UButton @click="handleFiltersApply" color="primary" class="w-full flex justify-center">
                Искать объекты
            </UButton>
        </div>
    </div>
...
const { data: filterTypesData, error: filterTypesError, refresh } = await useAsyncData(
    'filterTypes',
...
// Computed properties
const makeOptions = (key: keyof FilterTypes) => computed(() =>
...
const filteredBathroomOptions = computed(() =>
    bathroomTypeOptions.value.filter(option =>
        isFlat.value ? !['В доме', 'На улице'].includes(option.label) : !['Раздельный', 'Совмещенный'].includes(option.label)
    )
)
