    ...
    <div class="analytics-dashboard">
        ...
        <div class="sas">
            <div id="price-boxplot-chart" class="chart-container collapse">
                <apexchart type="boxPlot" height="500" :options="priceBoxplotChart.options"
                    :series="priceBoxplotChart.series">
                </apexchart>
            </div>
            <div id="area-boxplot-chart" class="chart-container collapse">
                <apexchart type="boxPlot" height="500" :options="areaBoxplotChart.options"
        ...
        <div class="charts-row-two">
            <div class="full-width-chart">
                <div class="bg-white rounded-xl  shadow p-6 h-full">
                    ...
                    <div v-else-if="priceHistoryChart.series.length > 0" id="price-history-chart"
                        class="chart-container">
                        <apexchart type="line" height="400" :options="priceHistoryChart.options"
                            :series="priceHistoryChart.series"></apexchart>
                    </div>
            ...
            <div class="full-width-chart">
                <div class="bg-white rounded-xl shadow p-6 h-full">
                    ...
                    <div v-else-if="viewsHistoryChart.series.length > 0" id="views-history-chart"
                        class="chart-container">
                        <apexchart type="line" height="400" :options="viewsHistoryChart.options"
                            :series="viewsHistoryChart.series"></apexchart>
                    </div>
                    <div v-else class="flex justify-center items-center py-8 text-gray-500">
                        Нет данных для отображения
                    </div>
            ...
            <div class="grid gap-2 h-fit">
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <div class="chart-toggle-buttons">
                        <UButton size="lg" color="neutral" variant="soft" @click="setActiveChart('priceCategories')"
                            :class="['toggle-btn', activeChart === 'priceCategories' ? 'active' : '']">
                            Ценовые категории
                        </UButton>
                        <UButton size="lg" color="neutral" variant="soft" @click="setActiveChart('avgPricePerSqm')"
                            :class="['toggle-btn', activeChart === 'avgPricePerSqm' ? 'active' : '']">
                            Средняя цена за м²
                        </UButton>
                        <UButton size="lg" color="neutral" variant="soft" @click="setActiveChart('viewsLastTenDays')"
                            :class="['toggle-btn', activeChart === 'viewsLastTenDays' ? 'active' : '']">
                            Просмотры за 10 дней
                        </UButton>
                        <UButton size="lg" color="neutral" variant="soft" @click="setActiveChart('priceBoxplot')"
                            :class="['toggle-btn', activeChart === 'priceBoxplot' ? 'active' : '']">
                            Распределение цен
                        </UButton>
                        <UButton size="lg" color="neutral" variant="soft" @click="setActiveChart('areaBoxplot')"
                            :class="['toggle-btn', activeChart === 'areaBoxplot' ? 'active' : '']">
                ...
                <div class="bg-white rounded-xl overflow-auto shadow p-6 h-fit">
                    ...
                    <div v-else-if="activeChart === 'priceCategories' && priceCategoriesChart.series.length > 0"
                        id="price-categories-chart" class="chart-container-group">
                        ...
                        <apexchart type="bar" :height="400"
                            ...
                            :options="priceCategoriesChart.options" :series="priceCategoriesChart.series">
                        </apexchart>

                    </div>
                    <div v-else-if="activeChart === 'avgPricePerSqm' && avgPricePerSqmChart.series.length > 0"
                        id="avg-price-per-sqm-chart" class="chart-container-group">
                        <apexchart type="bar" :height="400"
                            :width="avgPricePerSqmChart.options.xaxis.categories.length * 120 + 900"
                            :options="avgPricePerSqmChart.options" :series="avgPricePerSqmChart.series">
                        </apexchart>
                    </div>
                    <div v-else-if="activeChart === 'viewsLastTenDays' && viewsLastTenDaysChart.series.length > 0"
                        id="views-last-ten-days-chart" class="chart-container-group">
                        <apexchart type="bar" :height="400"
                            :width="viewsLastTenDaysChart.options.xaxis.categories.length * 120 + 900"
                            :options="viewsLastTenDaysChart.options" :series="viewsLastTenDaysChart.series">
                        </apexchart>
                    </div>
                    <div v-else-if="activeChart === 'priceBoxplot'" id="price-boxplot-chart"
                        class="chart-container-group">
                        <apexchart type="boxPlot" :height="400"
                            :width="priceBoxplotChart.options.xaxis.categories.length * 120 + 900"
                            :options="priceBoxplotChart.options" :series="priceBoxplotChart.series">
                        </apexchart>
                    </div>
                    <div v-else-if="activeChart === 'areaBoxplot'" id="area-boxplot-chart"
                        class="chart-container-group">
                        <apexchart type="boxPlot" :height="400"
                            :width="priceBoxplotChart.options.xaxis.categories.length * 120 + 900"
                            :options="areaBoxplotChart.options" :series="areaBoxplotChart.series">
                        </apexchart>
                    </div>
                    <div v-else class="flex justify-center items-center py-8 text-gray-500">
                        Нет данных для отображения
                ...
                <div class="popular-offers">
                    <div class="bg-white rounded-xl shadow p-6 h-full ">
                        ...
                        <div v-else class="space-y-4">
                            <div v-for="offer in totalStats?.top_offers_week" :key="offer.id"
                                ...
                                <div class="text-right ml-4">
                                    <p class="font-bold text-gray-900 text-sm">{{ formatPriceFull(offer.price_per_sqm)
                                        }}</p>
                                    <p class="text-xs text-gray-500">за м²</p>
                                </div>
                            </div>
                            <div v-if="totalStats?.top_offers_week.length === 0" class="text-center py-4 text-gray-500">
                                Нет популярных объявлений
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid gap-2">
                ...
                <div class="chart-third">
                    <div class="bg-white rounded-xl shadow p-6 h-full">
                        ...
                        <div v-else-if="propertyTypeChart.series.length > 0" id="property-type-chart"
                            class="chart-container">
                            <apexchart type="donut" height="250" :options="propertyTypeChart.options"
                                :series="propertyTypeChart.series"></apexchart>
                        </div>
                        <div v-else class="flex justify-center items-center py-8 text-gray-500">
                            Нет данных для отображения
                        </div>
                ...
                <div class="chart-third">
                    <div class="bg-white rounded-xl shadow p-6 h-full">
                        ...
                        <div v-else-if="roomsChart.series.length > 0" id="rooms-chart" class="chart-container">
                            <apexchart type="pie" height="250" :options="roomsChart.options"
                                :series="roomsChart.series">
                            </apexchart>
                        </div>
                        <div v-else class="flex justify-center items-center py-8 text-gray-500">
                            Нет данных для отображения
                ...
                <div class="chart-third">
                    <div class="bg-white rounded-xl shadow p-6 h-full">
                        ...
                        <div v-else-if="apartmentTypeChart.series.length > 0" id="apartment-type-chart"
                            class="chart-container">
                            <apexchart type="radialBar" height="250" :options="apartmentTypeChart.options"
                                :series="apartmentTypeChart.series"></apexchart>
                        </div>
                        <div v-else class="flex justify-center items-center py-8 text-gray-500">
                            Нет данных для отображения
                        </div>
...
const getCurrentGroupEntity = (item: SettlementData) => {
    switch (props.filters.groupBy) {
        ...
        case 'microdistrict': return item.microdistrict;
        case 'street': return item.street;
        default: return item.settlement;
    }
};
...
const prepareRoomsChartData = (data: SettlementData[]) => {
    ...
    const filteredData = labels.reduce((acc: { labels: string[], series: number[] }, label, index) => {
        if (series[index] > 0) {
...
const prepareAvgPricePerSqmChartData = (data: SettlementData[], categories: string[]) => {
    ...
    categories.forEach((category, index) => {
        ...
        if (item && item.avg_price_per_square_meter !== null &&
            item.avg_price_per_square_meter !== undefined &&
            item.avg_price_per_square_meter > 0) {
...
const prepareViewsLastTenDaysChartData = (data: SettlementData[], categories: string[]) => {
    ...
    categories.forEach((category, index) => {
        const item = data[index];
        if (item && item.last_ten_days_views_count !== undefined &&
            item.last_ten_days_views_count > 0) {
...
const preparePriceBoxplotChartData = (data: SettlementData[], categories: string[]) => {
    ...
    categories.forEach((category, index) => {
        const item = data[index];
        if (item && item.price_boxplot &&
            item.price_boxplot.min !== null &&
            item.price_boxplot.max !== null &&
...
const prepareAreaBoxplotChartData = (data: SettlementData[], categories: string[]) => {
    ...
    categories.forEach((category, index) => {
        const item = data[index];
        if (item && item.area_boxplot &&
            item.area_boxplot.min !== null &&
            item.area_boxplot.max !== null &&
