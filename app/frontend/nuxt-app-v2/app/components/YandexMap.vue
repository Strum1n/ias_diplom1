<template>
  ...
  >
    ...
    <!-- <yandex-map-controls :settings="{ position: 'bottom left', orientation: 'vertical' }">
      <yandex-map-control-button> Zoom: {{ zoom }} </yandex-map-control-button>
      <yandex-map-control-button>
    ...
    >
      <template #cluster="{length, coordinates}">
        ...
        <div
          v-if="
            clusterPopup &&
            areCoordsEqual(clusterPopup.coordinates, coordinates)
          "
          class="cluster-popup"
        >
          <div class="flex items-center justify-between mb-2">
            <p class="text-sm">Объявлений в доме: {{ length }}</p>
            <button class="cluster-close-btn" @click.stop="clusterPopup = null">
              ×
            </button>
          </div>
          ...
          <div class="offers-list">
            <div
              class="offer-card"
              v-for="(offer, index) in clusterPopup.offers"
              :key="offer.id"
            >
              ...
              <div class="relative">
                ...
                <button
                  ...
                  :class="{active: isFavorite(offer.id)}"
              ...
              <div
                class="gap-2 font-semibold text-[var(--color-info)] flex items-center"
              >
                {{ formatPrice(offer.price) }}

                <UBadge
                  v-if="offer.price_category == 'expensive'"
                  color="error"
                >
                  {{ getPriceCategoryLabel(offer.price_category) }}</UBadge
                >
                <UBadge
                  v-else-if="offer.price_category == 'normal'"
                  color="info"
                >
                  {{ getPriceCategoryLabel(offer.price_category) }}</UBadge
                >
                <UBadge
                  v-else="offer.price_category == 'cheap'"
                  color="success"
                >
                  {{ getPriceCategoryLabel(offer.price_category) }}</UBadge
                >
              ...
              <USeparator
                v-if="index < clusterPopup.offers.length - 1"
                class="my-3"
...
function areCoordsEqual(a: [number, number], b: [number, number]): boolean {
  return (
...
function getPricePopup(feature: ClustererFeature): string {
...
function getInfoPopup(feature: ClustererFeature): string {
  const filledIcon =
  ...
  const routeUrl = userLocation.value
    ? `https://yandex.ru/maps/?mode=routes&routes[activeComparisonMode]=auto&rtext=${userLocation.value?.lat},${userLocation.value?.lng}~${feature.geometry.coordinates[1]},${feature.geometry.coordinates[0]}&z=17`
    : `https://yandex.ru/maps/?mode=routes&routes[activeComparisonMode]=auto&rtext=~${feature.geometry.coordinates[1]},${feature.geometry.coordinates[0]}&z=17`;
  return feature.properties?.is_new_house
    ? `<div class="marker-popup">
        ...
        <div class="flex items-start justify-between gap-3">
            ...
            <a href="${routeUrl}" target="_blank" class="rounded-md font-medium inline-flex items-center disabled:cursor-not-allowed aria-disabled:cursor-not-allowed disabled:opacity-75 aria-disabled:opacity-75 transition-colors px-2.5 py-1.5 gap-1.5 text-inverted bg-inverted hover:bg-inverted/90 active:bg-inverted/90 disabled:bg-inverted aria-disabled:bg-inverted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-inverted mt-1.5 h-8 text-sm">
...
function openClusterPopup(clusterCoords: [number, number]) {
  ...
  const matchingOffers = props.parentOffers?.filter(offer => {
    ...
    return (
      Number(c[0].toFixed(6)) === rounded[0] &&
      Number(c[1].toFixed(6)) === rounded[1]
    );
  });
  ...
  if (prevFeature.value && prevFeature.value.properties?.isSelected == true) {
    console.log(
      `Меняем IsSelected у предыдущего маркера с id ${prevFeature.value?.id} с true на false`,
    );
    prevFeature.value.properties!.isSelected =
      !prevFeature.value.properties?.isSelected;
...
function openMarkerPopup(feature: ClustererFeature) {
  ...
  if (prevFeature.value && prevFeature.value.id != feature.id) {
    console.log(
      `Сейчас будем менять предыдущий маркер с id ${prevFeature.value?.id}`,
    );
    console.log(
      `У предыдущего маркера IsSelected это ${prevFeature.value?.properties?.isSelected}`,
    );
...
function createMarker(feature: ClustererFeature) {
  ...
  featureCircle.style.backgroundColor = getPriceCategoryColor(
    feature.properties?.price_category,
  );
  ...
  featureCircle.addEventListener("click", async e => {
    ...
    if (favoriteBtn) {
      ...
      if (offer) {
        ...
        const icon = favoriteBtn.querySelector(
          ".favorite-icon",
        ) as HTMLImageElement;
...
watch(userLocation, newLocation => {
  if (newLocation) {
    console.log("Локация пользователя обновилась:", newLocation);
  }
});
