<template>
    <yandex-map v-if="visible" v-model="map" cursor-grab :height="'100%'" :settings="{
        location: {
        ...
        <yandex-map-clusterer v-model="clusterer" :grid-size="64" :settings="{
                    ...
                    <div class="offers-list">
                        <div class="offer-card" v-for="(offer, index) in clusterPopup.offers" :key="offer.id">
                            ...
                            <div class="gap-2 font-semibold text-[var(--color-info)] flex items-center">{{
                                ...
                                <UBadge v-if="offer.price_category == 'expensive'" color='error'>
                                    {{ getPriceCategoryLabel(offer.price_category) }}</UBadge>
                                <UBadge v-else-if="offer.price_category == 'normal'" color='info'>
                                    {{ getPriceCategoryLabel(offer.price_category) }}</UBadge>
                                <UBadge v-else="offer.price_category=='cheap'" color='success'>
                                    {{ getPriceCategoryLabel(offer.price_category) }}</UBadge>
                            </div>

                            <div class="text-muted text-sm">{{ offer.address?.full_address }}</div>

                            <UButton class="w-max mt-1.5" size="md" color="info" trailing-icon="i-lucide-arrow-right"
                                :to="`/offers/${offer.id}`">
                                К объекту
                            </UButton>

                            <USeparator v-if="index < clusterPopup.offers.length - 1" class="my-3"
...
const getPriceCategoryColor = (category: string) => {
    switch (category) {
        case 'expensive': return 'var(--color-error)'
        case 'normal': return 'var(--color-info)'
        case 'cheap': return 'var(--color-success)'
        default: return 'gray'
    }
}
...
:deep(.favorite-heart), .favorite-heart {
