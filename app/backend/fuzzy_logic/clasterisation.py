...
def get_transport_access_category(value, consequent):
    ...
    memberships = {"poor": poor_membership, "average": average_membership, "good": good_membership}
...
metro = ctrl.Antecedent(np.arange(0, 1501, 1), "metro")
bus = ctrl.Antecedent(np.arange(0, 1001, 1), "bus")
...
metro["close"] = fuzz.trimf(metro.universe, [0, 0, 350])
metro["medium"] = fuzz.trimf(metro.universe, [2, 300, 1300])
metro["far"] = fuzz.trimf(metro.universe, [950, 1500, 1500])

# Задаем функции принадлежности для автобусных остановок
bus["close"] = fuzz.trimf(bus.universe, [0, 0, 250])
bus["medium"] = fuzz.trimf(bus.universe, [2, 100, 500])
bus["far"] = fuzz.trimf(bus.universe, [300, 1000, 1000])
...
# Создание правил
rules = [
    ctrl.Rule(metro["close"] & bus["medium"], transport_access["good"]),
    ctrl.Rule(metro["close"] & bus["close"], transport_access["good"]),
    ctrl.Rule(metro["medium"] & bus["medium"], transport_access["average"]),
]
