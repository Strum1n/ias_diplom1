...
async def check_identical_offers(
    ...
    async with session_factory() as session:
        ...
        conditions.append(
            _eq_or_isnull(
                Offer.living_area,
                float(living_area) if living_area is not None else None,
            )
        )
        conditions.append(
            _eq_or_isnull(
        ...
        conditions.append(
            _eq_or_isnull(
                ...
                int(house_floor_count) if house_floor_count is not None else None,
            )
        )
            ...
            try:
                ...
                if "uq_settlement_full_name" in str(e):
                    ...
                    new_offer.address.super_municipality = await get_existing(
                        session,
                        SuperMunicipality,
                        new_offer.address.super_municipality.name,
                    )
